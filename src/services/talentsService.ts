...
export function filterTalents(
  ...
  return talents.filter((talent) => {
    ...
    const matchesCategory =
      category === "Todos" ||
      (talent.data.status && talent.data.status.toLowerCase() === category.toLowerCase()) ||
      role.toLowerCase().includes(category.toLowerCase()) ||
      skills.some((skill) => skill.toLowerCase().includes(category.toLowerCase()));
 ...
 */
export function sortTalents(talents: TalentEntry[], sortBy: SortOption): TalentEntry[] {
