...
export function createCategorySwitch(
  ...
  const categoryButtons = Array.from(
    buttonsContainer.querySelectorAll<HTMLButtonElement>(
      `.${GALLERY_CLASSES.CATEGORY_BTN}`,
    ),
  );
  ...
  function updateActiveButton(activeBtn: HTMLButtonElement) {
    categoryButtons.forEach((btn) =>
      btn.classList.remove(GALLERY_CLASSES.ACTIVE),
    );
  ...
  function initialize(initialCategory: CategoryType = 'arte') {
    ...
    const initialButton = categoryButtons.find(
      (btn) => btn.dataset.cat === initialCategory,
    );
    if (initialButton) {
